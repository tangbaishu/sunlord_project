...
volatile unsigned char STATUS           @ 0x04;     //{ LVD24, 0, 0, PD, TO, DC, C, Z }
...
volatile unsigned char RSTSR            @ 0x09;     //{ 0, 0, 0, 0, 0, EMCF, ILOPF, WWDTF }
...
volatile unsigned char PT1              @ 0x20;     //{ 0, PT1[6:3], 0, PT1[1:0] }
volatile unsigned char PT1EN            @ 0x21;     //{ 0, PT1EN[6:3], 0, PT1EN[1:0] }
volatile unsigned char PT1PU            @ 0x22;     //{ 0, PT1PU[6:3], 0, PT1PU[1:0] }
...
volatile unsigned char PT3              @ 0x28;     //{ 0, 0, PT3[5:0] }
volatile unsigned char PT3EN            @ 0x29;     //{ 0, 0, PT3EN[5:0] }
volatile unsigned char PT3PU            @ 0x2A;     //{ 0, 0, PT3PU[5:0] }
volatile unsigned char PT3CON           @ 0x2B;     //{ 0, 0, PT3CON[5:0] }
...
volatile unsigned char METCH1           @ 0x2E;     //{ P3HINV, P3LINV, PT1W[6:4], PWM2PO, PWM2PO1, 0 }
volatile unsigned char METCH            @ 0x2F;     //{ VTHSEL, REF_SEL[2:0], PWMIS, T3RATE, T2RATE, P14_CUR }
...
volatile unsigned char SRADCON1         @ 0x35;     //{ SRADEN, SRADS, OFTEN, CALIF, ENOV, 0, VREFS[1:0] }
...
volatile unsigned char INTCFG           @ 0x40;     //{ LVD36, INTCFG_LVD24, 0, 0, 0, INTCFG[2:0] }
volatile unsigned char PT1CON1          @ 0x41;     //{ 0, 0, 0, 0, 0, 0, 0, PT1CON0 }
volatile unsigned char PT1PD            @ 0x42;     //{ 0, PT1PD[6:3], 0, PT1PD[1:0] }
volatile unsigned char PT3PD            @ 0x43;     //{ 0, 0, PT3PD[5:0] }
volatile unsigned char PT5PD            @ 0x44;     //{ 0, 0, 0, 0, 0, 0, PT5PD[1:0] }
volatile unsigned char TM3CON3          @ 0x45;     //{ PWM3PO, 0, 0, 0, P3H2INV, P3L2INV, P3H2OEN, P3L2OEN }
volatile unsigned char LCDCOM           @ 0x46;     //{ 0, 0, 0, LCDCOM[4:0] }
volatile unsigned char WWDTCR           @ 0x47;     //{ WWDTEN, TR[6:0] }
volatile unsigned char WWDTWR           @ 0x48;     //{ 0, WD[6:0] }
volatile unsigned char ISPCON1          @ 0x60;     //{ CHKRSLT, 0, 0, 0, 0, 0, 0, 0 }
volatile unsigned char WRPRT            @ 0x63;     //{ 0, 0, 0, 0, 0, 0, 0, WRPRTF }
volatile unsigned char CMPCON0          @ 0x6A;     //{ CMPEN, INSHTEN, HYSEN, CMP_PSEL, CMP_NSEL[2:0], CMPOUT0 }
volatile unsigned char CMPCON1          @ 0x6B;     //{ CMPOEN, CMPVTHEN, CMPVTHS[1:0], VTHOEN, CMP_FLT[2:0] }
volatile unsigned char CMPCON2          @ 0x6C;     //{ CMPENS[1:0], CMPINTS[1:0], 0, 0, TM3STP, TM2STP }
volatile unsigned char CCSCON           @ 0x6D;     //{ 0, 0, 0, 0, 0, 0, 0, CCSOEN }
volatile unsigned char METCH2           @ 0x79;     //{ 0, 0, 0, 0, METCH2[3:0] }
...
//-------------------------------------------------------
volatile sbit TM3R_0                    @ 0x1E*8+0;
volatile sbit TM3R_1                    @ 0x1E*8+1;
volatile sbit TM3R_2                    @ 0x1E*8+2;
volatile sbit TM3R_3                    @ 0x1E*8+3;
volatile sbit TM3R_4                    @ 0x1E*8+4;
volatile sbit TM3R_5                    @ 0x1E*8+5;
volatile sbit TM3R_6                    @ 0x1E*8+6;
volatile sbit TM3R_7                    @ 0x1E*8+7;
...
//-------------------------------------------------------
volatile sbit PWM2PO1                   @ 0x2E*8+1;
...
//-------------------------------------------------------
volatile sbit P14_CUR                   @ 0x2F*8+0;
volatile sbit T2RATE_3                  @ 0x2F*8+1;
volatile sbit T3RATE_3                  @ 0x2F*8+2;
...
volatile sbit CMPIF                     @ 0x3C*8+7;

//-------------------------------------------------------
//  INTE2 register bit map
//-------------------------------------------------------
volatile sbit TM3IE                     @ 0x3D*8+4;
volatile sbit CMPIE                     @ 0x3D*8+7;
...
volatile sbit INTCFG_LVD24                @ 0x40*8+6;
volatile sbit LVD36                     @ 0x40*8+7;

//-------------------------------------------------------
//  PT1CON1 register bit map
//-------------------------------------------------------
volatile sbit PT1CON0                   @ 0x41*8+0;

//-------------------------------------------------------
//  PT1PD register bit map
//-------------------------------------------------------
volatile sbit PT1PD_0                   @ 0x42*8+0;
volatile sbit PT1PD_1                   @ 0x42*8+1;
volatile sbit PT1PD_3                   @ 0x42*8+3;
volatile sbit PT1PD_4                   @ 0x42*8+4;
volatile sbit PT1PD_5                   @ 0x42*8+5;
volatile sbit PT1PD_6                   @ 0x42*8+6;

//-------------------------------------------------------
//  PT3PD register bit map
//-------------------------------------------------------
volatile sbit PT3PD_0                   @ 0x43*8+0;
volatile sbit PT3PD_1                   @ 0x43*8+1;
volatile sbit PT3PD_2                   @ 0x43*8+2;
volatile sbit PT3PD_3                   @ 0x43*8+3;
volatile sbit PT3PD_4                   @ 0x43*8+4;
volatile sbit PT3PD_5                   @ 0x43*8+5;

//-------------------------------------------------------
//  PT5PD register bit map
//-------------------------------------------------------
volatile sbit PT5PD_0                   @ 0x44*8+0;
volatile sbit PT5PD_1                   @ 0x44*8+1;
...
volatile sbit PWM3PO                    @ 0x45*8+7;

//-------------------------------------------------------
//  LCDCOM register bit map
//-------------------------------------------------------
volatile sbit LCDCOM_0                  @ 0x46*8+0;
volatile sbit LCDCOM_1                  @ 0x46*8+1;
volatile sbit LCDCOM_2                  @ 0x46*8+2;
volatile sbit LCDCOM_3                  @ 0x46*8+3;
volatile sbit LCDCOM_4                  @ 0x46*8+4;

//-------------------------------------------------------
//  WWDTCR register bit map
//-------------------------------------------------------
volatile sbit TR_0                      @ 0x47*8+0;
volatile sbit TR_1                      @ 0x47*8+1;
volatile sbit TR_2                      @ 0x47*8+2;
volatile sbit TR_3                      @ 0x47*8+3;
volatile sbit TR_4                      @ 0x47*8+4;
volatile sbit TR_5                      @ 0x47*8+5;
volatile sbit TR_6                      @ 0x47*8+6;
volatile sbit WWDTEN                    @ 0x47*8+7;

//-------------------------------------------------------
//  WWDTWR register bit map
//-------------------------------------------------------
volatile sbit WD_0                      @ 0x48*8+0;
volatile sbit WD_1                      @ 0x48*8+1;
volatile sbit WD_2                      @ 0x48*8+2;
volatile sbit WD_3                      @ 0x48*8+3;
volatile sbit WD_4                      @ 0x48*8+4;
volatile sbit WD_5                      @ 0x48*8+5;
volatile sbit WD_6                      @ 0x48*8+6;

//-------------------------------------------------------
//  ISPCON1 register bit map
//-------------------------------------------------------
volatile sbit CHKRSLT                   @ 0x60*8+7;
...
//-------------------------------------------------------
volatile sbit CMPOUT0                   @ 0x6A*8+0;
volatile sbit CMP_NSEL_0                @ 0x6A*8+1;
volatile sbit CMP_NSEL_1                @ 0x6A*8+2;
volatile sbit CMP_NSEL_2                @ 0x6A*8+3;
volatile sbit CMP_PSEL                  @ 0x6A*8+4;
volatile sbit HYSEN                     @ 0x6A*8+5;
volatile sbit INSHTEN                   @ 0x6A*8+6;
volatile sbit CMPEN                     @ 0x6A*8+7;

//-------------------------------------------------------
//  CMPCON1 register bit map
//-------------------------------------------------------
volatile sbit CMP_FLT_0                 @ 0x6B*8+0;
volatile sbit CMP_FLT_1                 @ 0x6B*8+1;
volatile sbit CMP_FLT_2                 @ 0x6B*8+2;
volatile sbit VTHOEN                    @ 0x6B*8+3;
volatile sbit CMPVTHS_0                 @ 0x6B*8+4;
volatile sbit CMPVTHS_1                 @ 0x6B*8+5;
volatile sbit CMPVTHEN                  @ 0x6B*8+6;
volatile sbit CMPOEN                    @ 0x6B*8+7;

//-------------------------------------------------------
//  CMPCON2 register bit map
//-------------------------------------------------------
volatile sbit TM2STP                    @ 0x6C*8+0;
volatile sbit TM3STP                    @ 0x6C*8+1;
volatile sbit CMPINTS_0                 @ 0x6C*8+4;
volatile sbit CMPINTS_1                 @ 0x6C*8+5;
volatile sbit CMPENS_0                  @ 0x6C*8+6;
volatile sbit CMPENS_1                  @ 0x6C*8+7;

//-------------------------------------------------------
//  CCSCON register bit map
//-------------------------------------------------------
volatile sbit CCSOEN                    @ 0x6D*8+0;

//-------------------------------------------------------
//  METCH2 register bit map
//-------------------------------------------------------
volatile sbit METCH2_0                  @ 0x79*8+0;
volatile sbit METCH2_1                  @ 0x79*8+1;
volatile sbit METCH2_2                  @ 0x79*8+2;
volatile sbit METCH2_3                  @ 0x79*8+3;
