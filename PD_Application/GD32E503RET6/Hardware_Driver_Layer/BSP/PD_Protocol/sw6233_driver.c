...
void SW6233_Init(void)
{
	...
	IIC_Driver_Write_Single_Data(DEVICE_ADDRESS, 0x40, 0x01);	// ADC配置 输入/输出 IBUS 电流 单位:25/11=2.27 mA/step
	...
	LED_Function_List.LED_Function_ON(1);						// 初始化完成状态指示灯

	while (1)
	{
		SW6233_Working_State_Check();1
		delay_1ms(200);
	}
...
void Close_All_Besides_PD(void)
{
	...
	ram_data |= 0x08;
	IIC_Driver_Write_Single_Data(DEVICE_ADDRESS, 0xB6, ram_data);	// 端口检测配置 2 -> 关闭 C1/C2 口 DM 接入检测
	
	IIC_READ_SINGLE_DATA(DEVICE_ADDRESS, 0xB1, &ram_data);
	ram_data |= 0xEC;
	IIC_Driver_Write_Single_Data(DEVICE_ADDRESS, 0xB1, ram_data);	// 快充协议开关 1 -> 关闭该寄存器下的所有协议

	IIC_READ_SINGLE_DATA(DEVICE_ADDRESS, 0xCD, &ram_data);
	ram_data |= 0xF8;
	IIC_Driver_Write_Single_Data(DEVICE_ADDRESS, 0xCD, ram_data);	// 快充协议开关 2 -> 关闭该寄存器下的所有协议
}
